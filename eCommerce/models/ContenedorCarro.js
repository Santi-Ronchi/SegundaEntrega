...
class ContenedorCarro {
    ...
    async saveCarro(obj){
        try{
            ...
            fs.writeFileSync(this.path,JSON.stringify([{...obj, prodList: [], time: Date.now(), id: 1}]))
            return (1);
        }
   ...
   async deleteProductById(cartId, prodId) {
        ...
        if (cart) {
          const productIndex = cart.prodList.findIndex(p => p.id === parseInt(prodId));
          if (productIndex > -1) {
            cart.prodList.splice(productIndex, 1);
            await fs.promises.writeFile(this.path, JSON.stringify(carros, null, 2));
